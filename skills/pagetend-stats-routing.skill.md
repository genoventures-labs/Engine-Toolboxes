...
3. For scheduling / queue questions use `dashboard`, then verify `fallback` is not `true` before trusting figures.
4. Keep `summary: true` (default) so responses stay compact; dig into `data` only when needed.
5. Cite which endpoint the numbers came from and whether Meta was connected.
